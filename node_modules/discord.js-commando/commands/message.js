...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`\`${this.command.name}\` Komutları sunucuda deneyiniz..`);
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.reply(`\`${this.command.name}\` komut sadece NSFW kanallarında kullanılabilir.`);
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`\`${this.command.name}\` Komutunu kullanmak için yetkin yetersiz.`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.reply(
						`\`${this.command.name}\` Bu komut kullanmak için "${permissions[missing[0]]}" yetkisine ihtiyacım var.`
					);
				}
				return this.reply(oneLine`
				\`${this.command.name}\` Komutunu kullanmak için aşağıdaki izinlere ihtiyacım var:
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`\`${this.command.name}\` Bu komutu kullanamazsınız tekrar kullanmak için ${remaining.toFixed(1)} saniye bekleyin.`
			);
		}
		...
		try {
			this.client.emit('debug', `Komutu kullanıldı ${this.command.groupID}:${this.command.memberName}.`);
			...
			if(err instanceof FriendlyError) {
				...
				return this.reply(stripIndents`
					Bir hata oluştu: \`${err.name}: ${err.message}\`
					Hatayı bot yapımcısına bildiriniz.
					Bot sahibi ${ownerList || ''}${invite ? ` Sunucusu: ${invite}` : '.'}
				`);
			}
		}
	}
